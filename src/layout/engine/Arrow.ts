...
export class Arrow {
  ...
  fit(tail: Box, head: Box, dx: number, dy: number): Box {
    function cut(v: Vector2d, box: Box): Vector2d {
      const candidates: Vector2d[] = [];
      if (v.dx < 0) {
        const dx = v.x1 - box.cx + box.width / 2;
        const ratio = -dx / v.dx;
        candidates.push(v.multiple(ratio));
      } else if (v.dx > 0) {
        const dx = box.cx - v.x1 + box.width / 2;
        const ratio = dx / v.dx;
        candidates.push(v.multiple(ratio));
      }
      if (v.dy < 0) {
        const dy = v.y1 - box.cy + box.height / 2;
        const ratio = -dy / v.dy;
        candidates.push(v.multiple(ratio));
      } else if(v.dy > 0) {
        const dy = box.cy - v.y1 + box.height / 2;
        const ratio = dy / v.dy;
        candidates.push(v.multiple(ratio));
      }
